...
static STimeWindow stTriggerTaskGetPeriodWindow(const SInterval *pInterval, int64_t ts) {
  ...
  if (pInterval->sliding > day) {
    ...
    if (first >= ts) {
      ...
      win.ekey = win.skey + pInterval->sliding;
    }
  }
  return win;
}
...
static int32_t stRealtimeGroupInit(SSTriggerRealtimeGroup *pGroup, SSTriggerRealtimeContext *pContext, int64_t gid) {
  ...
  if (pContext->pTask->isVirtualTable) {
    pGroup->pVirTableInfos = tSimpleHashInit(0, taosGetDefaultHashFunction(TSDB_DATA_TYPE_BIGINT));
    QUERY_CHECK_NULL(pGroup->pVirTableInfos, code, lino, _end, terrno);
    tSimpleHashSetFreeFp(pGroup->pVirTableInfos, stRealtimeGroupDestroyTableInfo);
  }
  pGroup->pTableMetas = tSimpleHashInit(0, taosGetDefaultHashFunction(TSDB_DATA_TYPE_BIGINT));
...
static void stRealtimeGroupDestroy(void *ptr) {
  ...
  if (pGroup->pVirTableInfos != NULL) {
    tSimpleHashCleanup(pGroup->pVirTableInfos);
...
static void stRealtimeGroupFinish(SSTriggerRealtimeGroup *pGroup) {
  int32_t             iter = 0;
  SSTriggerTableMeta *pTableMeta = tSimpleHashIterate(pGroup->pTableMetas, NULL, &iter);
  while (pTableMeta != NULL) {
    int32_t metaIdx =
        taosArraySearchIdx(pTableMeta->pMetas, &pGroup->newThreshold, stRealtimeGroupMetaDataSearch, TD_GT);
    taosArrayPopFrontBatch(pTableMeta->pMetas, metaIdx);
    for (int32_t j = 0; j < TARRAY_SIZE(pTableMeta->pMetas); j++) {
      ...
      if (pMeta->skey <= pGroup->newThreshold) {
        pMeta->skey = pGroup->newThreshold + 1;
...
static int32_t stRealtimeGroupAddMetaDatas(SSTriggerRealtimeGroup *pGroup, SSDataBlock *pDataMeta) {
  int32_t                   code = TSDB_CODE_SUCCESS;
  int32_t                   lino = 0;
  SSTriggerRealtimeContext *pContext = pGroup->pContext;
  SStreamTriggerTask       *pTask = pContext->pTask;
  SSTriggerTableMeta       *pTableMeta = NULL;
  SSTriggerTableMetaList    tablesToCheck = {0};
  ...
  for (int32_t i = 0; i < numNewMeta; i++) {
    ...
    if (pEkeys[i] > pGroup->oldThreshold) {
      ...
      if (TD_DLIST_NODE_NEXT(pTableMeta) == NULL && TD_DLIST_TAIL(&tablesToCheck) != pTableMeta) {
        TD_DLIST_APPEND(&tablesToCheck, pTableMeta);
      }
  ...
  if (TD_DLIST_NELES(&tablesToCheck) == 0) {
    // no new metadata to process
    goto _end;
  }

  while (TD_DLIST_NELES(&tablesToCheck) > 0) {
    pTableMeta = TD_DLIST_HEAD(&tablesToCheck);
    taosArraySort(pTableMeta->pMetas, stRealtimeGroupMetaDataCompare);
    while (TARRAY_SIZE(pTableMeta->pMetas) > 0) {
    ...
    TD_DLIST_POP(&tablesToCheck, pTableMeta);
  }
  ...
  while (pTableMeta != NULL) {
    maxNumHold = TMAX(maxNumHold, TARRAY_SIZE(pTableMeta->pMetas));
    pTableMeta = tSimpleHashIterate(pGroup->pTableMetas, pTableMeta, &iter);
  }
  ...
  while (pTableMeta != NULL) {
    if (TARRAY_SIZE(pTableMeta->pMetas) >= numHoldThreshold) {
      SSTriggerMetaData *pMeta = taosArrayGetLast(pTableMeta->pMetas);
...
static int32_t stRealtimeGroupSaveInitWindow(SSTriggerRealtimeGroup *pGroup, SArray *pInitWindows) {
  ...
  if (IS_REALTIME_GROUP_OPEN_WINDOW(pGroup)) {
    for (SSTriggerWindow *p = &TRINGBUF_FIRST(&pGroup->winBuf); p != &TRINGBUF_LAST(&pGroup->winBuf);) {
...
static int32_t stRealtimeGroupRestoreInitWindow(SSTriggerRealtimeGroup *pGroup, SArray *pInitWindows) {
  ...
  if (pTask->triggerType == STREAM_TRIGGER_SLIDING) {
    ...
    pGroup->nextWindow = *(STimeWindow *)taosArrayPop(pInitWindows);
    nWindows--;
  }
...
static int32_t stRealtimeGroupMergeSavedWindows(SSTriggerRealtimeGroup *pGroup, int64_t gap) {
  ...
  for (int32_t i = 0; i < TARRAY_SIZE(pContext->pSavedWindows); i++) {
    ...
    if ((calcOpen || notifyOpen) && i >= nInitWins) {
      SSTriggerCalcParam param = {.triggerTime = taosGetTimestampMs(),
  ...
  *pNeedFetchData = false;
  ...
  while (!*pAllTableProcessed && !*pNeedFetchData) {
    if (!pTask->isVirtualTable) {
      ...
      if (IS_TRIGGER_VTABLE_MERGER_EMPTY(pContext->pSorter)) {
        while (saveWindow && IS_REALTIME_GROUP_OPEN_WINDOW(pGroup)) {
          code = stRealtimeGroupCloseWindow(pGroup, NULL, saveWindow);
          QUERY_CHECK_CODE(code, lino, _end);
        }
        stVtableMergerReset(pContext->pMerger);
        pGroup->pCurVirTable = tSimpleHashIterate(pGroup->pVirTableInfos, pGroup->pCurVirTable, &pGroup->tbIter);
        if (pGroup->pCurVirTable == NULL) {
          *pAllTableProcessed = true;
          break;
        }
        if (saveWindow) {
...
static int32_t stRealtimeGroupDoSlidingCheck(SSTriggerRealtimeGroup *pGroup) {
  ...
  if (IS_REALTIME_GROUP_NONE_WINDOW(pGroup)) {
    ...
    code = stRealtimeGroupSaveInitWindow(pGroup, pContext->pSavedWindows);
    QUERY_CHECK_CODE(code, lino, _end);
  }
...
static int32_t stRealtimeContextInit(SSTriggerRealtimeContext *pContext, SStreamTriggerTask *pTask) {
  ...
  QUERY_CHECK_CODE(code, lino, _end);
  if (pTask->isVirtualTable) {
    // todo(kjq): retrieve from virt table infos
...
static int32_t stRealtimeContextSendPullReq(SSTriggerRealtimeContext *pContext, ESTriggerPullType type) {
  ...
  switch (type) {
    ...
    case STRIGGER_PULL_WAL_META: {
      ...
      pReq->ctime = INT64_MAX;  // todo(kjq): set the limit for period trigger
      break;
    }
    ...
    case STRIGGER_PULL_WAL_CALC_DATA: {
      ...
      SSTriggerTableMeta             *pCurTableMeta = TD_DLIST_HEAD(&pContext->groupsToCheck)->pCurTableMeta;
    ...
    case STRIGGER_PULL_WAL_DATA: {
      ...
      SSTriggerTableMeta      *pCurTableMeta = tSimpleHashGet(TD_DLIST_HEAD(&pContext->groupsToCheck)->pTableMetas,
                                                              &pColRefToFetch->otbUid, sizeof(int64_t));
      ...
      pReq->ekey = pMetaToFetch->ekey;
      if (pReq->cids == NULL) {
        pReq->cids = taosArrayInit(0, sizeof(col_id_t));
        QUERY_CHECK_NULL(pReq->cids, code, lino, _end, terrno);
      }
      *(col_id_t *)TARRAY_DATA(pReq->cids) = 0;
    ...
    case STRIGGER_PULL_GROUP_COL_VALUE: {
      ...
      if (pTask->isVirtualTable) {
        int32_t                iter = 0;
        SSTriggerVirTableInfo *pTable = tSimpleHashIterate(pGroup->pVirTableInfos, NULL, &iter);
...
static int32_t stRealtimeContextSendCalcReq(SSTriggerRealtimeContext *pContext) {
  ...
  for (int32_t i = 0; i < TARRAY_SIZE(pCalcReq->params); ++i) {
    ...
    ST_TASK_ILOG("[calc param %d]: gid=%" PRId64 ", wstart=%" PRId64 ", wend=%" PRId64 ", nrows=%" PRId64 ", create=%d",
                 i, pCalcReq->gid, pParam->wstart, pParam->wend, pParam->wrownum, pCalcReq->createTable);
  }
  ...
  switch (pReq->type) {
    ...
    case STRIGGER_PULL_WAL_META: {
      ...
      if (pRsp->code == TSDB_CODE_WAL_LOG_NOT_EXIST) {
        ...
        // todo(kjq): it should never happen
        blockDataEmpty(pDataBlock);
        SStreamTaskAddr *pReader = taosArrayGet(pTask->readerList, pContext->curReaderIdx);
        QUERY_CHECK_NULL(pReader, code, lino, _end, terrno);
        SSTriggerWalProgress *pProgress =
        ...
        QUERY_CHECK_NULL(pProgress, code, lino, _end, TSDB_CODE_INTERNAL_ERROR);
        pDataBlock->info.id.groupId = pProgress->latestVer;
      }
    ...
    case STRIGGER_PULL_WAL_DATA: {
      ...
      SSDataBlock *pDataBlock = taosMemoryCalloc(1, sizeof(SSDataBlock));
      QUERY_CHECK_NULL(pDataBlock, code, lino, _end, terrno);
      if (pRsp->contLen > 0) {
        const char *pCont = pRsp->pCont;
        code = blockDecode(pDataBlock, pCont, &pCont);
...
int32_t stTriggerTaskAcquireRequest(SStreamTriggerTask *pTask, int64_t sessionId, int64_t gid,
  ...
  *ppRequest = NULL;
  ...
  pRunningFlag[0] = pRunningFlag[idx + 1] = true;
